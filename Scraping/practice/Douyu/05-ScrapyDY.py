...
def get_info(source,Tname):
	...
	for page in range(1, 121):
		...
		print("{}——正在爬取第{}个主播!".format(Tname, Count_num))
		# print("名字：", dy_name)
		# print("分类：", dy_type)
		# print("标题：", dy_title)
		# print("热度：", dy_num )
...
def get_one_type(url_, name):
	...
	while Flag:
		...
		tName ="thread-{}".format(page)
		pages = driver.page_source		
		t = threading.Thread(target=get_info, name=tName, args=(pages,threading.Thread.name))
		threads.append(t)
		try:
...
def main():
	...
	start = 0
	for name, url in urls.items():
		t = threading.Thread(target=get_one_type,args=(url,name))
		threads.append(t)
	s_time = time()
	count = 0
	add = 4   #增量
